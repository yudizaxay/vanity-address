...
> **Tip:** `*` = version number (e.g. `0.4.0`). Prefer [latest release](https://github.com/yudizaxay/vanity-address/releases/latest) for the newest filenames.  
...
```bash
# Replace 0.4.0 with the version on the Releases page if newer
...
```bash
curl -LO https://github.com/yudizaxay/vanity-address/releases/download/v0.4.0/VanityAddress-0.4.0-Mac-AppleSilicon-CLI.tar.gz
tar xzf VanityAddress-0.4.0-Mac-AppleSilicon-CLI.tar.gz
./vanity-address
```
