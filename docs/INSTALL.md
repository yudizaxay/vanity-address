...
> **Tip:** `*` = version number (e.g. `0.6.0`). Prefer [latest release](https://github.com/yudizaxay/vanity-address/releases/latest) for the newest filenames.  
...
```bash
# Replace 0.6.0 with the version on the Releases page if newer
...
```bash
curl -LO https://github.com/yudizaxay/vanity-address/releases/download/v0.6.0/VanityAddress-0.6.0-Mac-AppleSilicon-CLI.tar.gz
tar xzf VanityAddress-0.6.0-Mac-AppleSilicon-CLI.tar.gz
./vanity-address
```
...
shasum -a 256 -c VanityAddress-0.6.0-Linux-CLI.tar.gz.sha256
```

```powershell
# Windows (optional) — compare hash manually
Get-FileHash VanityAddress-0.6.0-Windows-CLI.zip -Algorithm SHA256
```
